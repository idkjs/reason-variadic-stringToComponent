...
let make = () => {
  <div className="contactform-container">
    <div>
      <span> {React.string("Type on latin keyboard :  ")} </span>
      <button id="breset"> {React.string("Reset")} </button>
      <button id="bcopy"> {React.string("Copy to clipboard")} </button>
      <br />
      <input type_="text" size=100 id="entree" autoFocus=true />
    </div>
  </div>;
};
